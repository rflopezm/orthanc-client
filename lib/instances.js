...
const Instances = {
  ...
  putMetadata: function(client, id, metadata) {
    return new Promise((resolve, reject) => {
      request.put(
        {
          url: client.url + '/instances/' + id + '/metadata/' + metadata.name,
          body: metadata.value,
          auth: client.auth
        },
        function(error, response, body) {
   ...
   * @param {String} id Id of the targeted instance
   */
  modify: function(client, id, manipulation) {
    //TODO
    return new Promise((resolve, reject) => {
      request.post(
        {
          url: `${client.url}/instances/${id}/modify`,
